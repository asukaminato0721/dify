...
When you port api to api-new, always read the roadmap.txt, and ask subagent to port them.

after port, finish the test, update the roadmap.txt

Use good library if possible, you dont need to stick on the old libraries, use the fastapi eco libraries.

always use full type python, don't just return Mapping, return TypedDict instead.
